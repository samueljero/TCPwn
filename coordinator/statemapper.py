...
class StateMapper():
    ...
    actionMap = {
        ...
        "ACK":[{'action':'FORCEACK','param':'*'},{}],
        #Dup Acks implemented with DUP action
        "ACK && dup":[{'action':'DUP','param':'num=100'}],
        "ACK && dup && dupACKctr < 2":[{'action':'DUP','param':'num=1'}],
        "ACK && dup && dupACKctr+1 == 3":[{'action':'DUP','param':'num=4'}],
        #Send current ACK. Also just wait for protocol
        "ACK && new":[{'action':'FORCEACK','param':'*'},{}],
        ...
        "ACK && new && cwnd+MSS < ssthresh":[{'action':'FORCEACK','param':'*'},{}],
        ...
        #Burst interrupts timing, making RTO likely. Also some sort of drop action?
        "RTO Timeout":[{'action':'BURST','param':'num=10'}],
    }
